...
`.map` allocates memory and stores return values in new array. `.forEach` throws away the return values and returns undefined to allow a callback function to mutate the current array.
...
It is needed in order to call the methods of the parent class.
